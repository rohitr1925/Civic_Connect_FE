...
const TeacherComplain = () => {
  ...
  const school = currentUser.school?._id || currentUser.admin?._id || '';
  ...
  const Pcomplains = () => {
    ...
    const complainList = complainsList.filter((complain)=> complain.status==='Pending');
  ...
  const Ccomplains = () => {
    ...
    const complainList = complainsList.filter((complain)=> complain.status==='Completed');
    const complainRows = complainList && complainList.length > 0 && complainList.map((complain) => {
      const date = new Date(complain.date);
      const dateString = date.toString() !== "Invalid Date" ? date.toISOString().substring(0, 10) : "Invalid Date";
      return {
